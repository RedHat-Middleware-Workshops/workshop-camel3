...
{empty} +
+
In the filter above you'll notice the expression picks the `$.text` JSON field and evaluates it against the regular expression that takes in account the _HTML_ `<b>` tags _Gitter_ uses to render rich content.
+
{empty} + 
...
+
```
<b>fakebot@gitter</b>: fake app message
```
+
{empty} +
+
