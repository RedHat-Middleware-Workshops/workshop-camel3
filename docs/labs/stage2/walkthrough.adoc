...
[NOTE]
--
The filter is simpler, because _Rocket.Chat_ includes a `botId` field when messages are app-generated.
...
. Include an extra _Rocket.Chat_ parameter to your properties file.
+
Copy from below the parameter definition `rocketchat.channel.name`, paste it into your properties file, and set its value to the _Rocket.Chat_ room you're using.
+
----
# Matrix credentials
...
+
```properties
rocketchat.channel.name=YOUR_SLACK_ROOM
```
+
{blank}
+
An example setting for the channel name could be `rocketchat.channel.name=room1`.
+
{empty} +
