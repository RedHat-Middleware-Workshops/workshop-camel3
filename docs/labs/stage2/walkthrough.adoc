...
For example:
+
** *{user-username}@rocketchat:* hello
+
{blank}
+
--
...
. Include the filter
+
_RocketChat_ does not provide a field to flag bot-generated messages, so we need to develop a custom pattern-matching approach to distinguish user messages from bot messages.
+
--
The data mapper transforms messages to the following format:
