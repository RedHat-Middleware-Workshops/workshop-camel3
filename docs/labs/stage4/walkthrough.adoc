...
+
We provide the base sink Kamelet for you to work on. Find in your directory tree the following resource:
+
...
--
+
NOTE: The kamelet defines an input parameter `token` and its value gets used in the header.
====
+
{empty} +
...
--
+
NOTE: The kamelet defines an input parameter `channel` and its value gets used in the header.
====
+
{empty} +
...
+
image::images/crw-folder1.png[align="left", width=30%]
+
{blank}
+
...
- `token`
+
Obtain the _Discord_ token following one of the options listed below, and replace `YOUR_TOKEN`:
+
=====

** *Option 1*: Try in _Rocket.Chat_:
+
They may be available in _Rocket.Chat_. +
Look in the `#general` channel:
+
image::images/rc-discord-credentials.png[align="left", width=90%]
+
=========
...
** *Option 3*: Ask your instructor/administrator for the token to use to connect to Discord.
+
[TIP]
====
...
====
** *Option 4*: if you created your own _Discord_ App, use your App's `Bot User OAuth Token`
=====
...
[IMPORTANT]
====
Ensure you only pick the last token (after last `/`) from the address bar.
====
--
+
{empty} +
...
. Define your Kamelet
+
.. Define your Kamelet metadata and parameters
+
(Copy and replace the `spec`, keep the _Kamelet_'s header/metadata.)
+
----
...
+
(Copy/paste below `spec`, at the bottom of the _Kamelet_)
+
NOTE: This snippet below is the one you have just tested locally.
+
```yaml
      - remove-headers:
          pattern: "*"
...
The commands below clones the _Kafka_ to _Rocket.Chat_ and modifies it to use _Discord_ instead.
+
...
====
* *Kamelets* +
These are like capsules containing _Camel_ routes that can be used in _Kamalet_ bindings and also invoked from _Camel_'s DSL (_Domain Specific Language_).

* *Kamelet bindings* +
Bindings are no-code definitions where _Kamelets_ are bound together or to a platform resource (_Kafka/Knative_). _Camel K_ deploys them as a running integration units.
...
. Find your new folder in the project tree
+
image::images/crw-folder2.png[align="left", width=30%]
+
{blank}
+
...
+
Inlcude in your `d2k.xml` the additional _Camel_ route:
+
```xml
    <route id="heartbeat">
...
+
Inlcude in your `d2k.xml` the yet another _Camel_ route:
+
```xml
    <route id="listener">
...
. Include a _Kafka_ publisher
+
Inlcude in your `d2k.xml` the last _Camel_ route:
+
```xml
...
----
+
NOTE: You can read the in the logs the message `bypassing kafka push`. The code you copied contains logic to bypass _Kafka_ when running locally with _Camel JBang_.
+
{empty} +
