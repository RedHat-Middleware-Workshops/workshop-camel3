...
//:openshift-app-host: apps.cluster-dspt2.dynamic.redhatworkshops.io
:url-invite: https://invite-invite.{openshift-app-host}/status2
//:url-invite: http://0.0.0.0:8080/status2
...
+
Camel JBang is packed with handy functionality. One of them is a nano editor you can use for simple edits. +
Copy and paste the command below to edit the file using Camel JBang in-built editor:
+
[source, subs=]
----
camel nano m2r.camel.yaml<br>
----
+
{blank}
+
The Nano editor will present on screen. +
Copy and paste the following snippet:
+
[source, subs=]
----
- route:
    description: Matrix to RocketChat
    from:
      description: Matrix
      uri: kamelet
      parameters:
...
{blank}
+
Then use the sequence of keyboard combinations below:
+
- kbd:[Ctrl+V] to paste the snippet
- kbd:[Ctrl+X] to exit the nano editor and kbd:[Y] to save the changes
- kbd:[Enter] to confirm the file name.
+
{empty} +

. Define the configuration
+
Include in your properties file `stage1.properties` the following entries:
+
```properties
...
.. Click first the Attach schema button, then the blue button, and pick `matrix-schema.json`.
+
{blank}
+
. Define the target
