...
[IMPORTANT] 
--
You need to already be onboarded into _Gitter_ and _Slack_. +
...
====
+
NOTE: The `gitter-source` kamelet is implemented following the specification of the Matrix Sync API (new Gitter's API) to consume events from the server. To know more, read its API documentation here: https://spec.matrix.org/v1.6/client-server-api/#syncing
+
{empty} +
...
----
+
CAUTION: If your terminal does not log JSON events (as above), you might be sending messages from the _Matrix_ server instead of the _Gitter_ server. Ensure you connect to the _Gitter_ server when you login to the platform from your browser. Follow carefully the instructions in the section *Gitter platform onboarding* where the login process is described in detail.
+
{blank}
+
...
+
----
{"channel":"TO_BE_DEFINED","text":"*bmesegue@gitter*: Hello Camel"}
----
+
{empty} +
+
