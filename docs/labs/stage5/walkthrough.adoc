...
pass:[<mark style="padding-left: 2rem; background-color: white; color: purple"><b><i>&nbsp;!&nbsp;&nbsp;</i></b></mark>]*store.camel.yaml*
--
...
. One route as the **main-processor** _Camel_ as a registered bot.
. One route to wait until aggregations are fished.
. One route to store data to S3.
====
+
[NOTE]
====
...
+
Append in your `store.camel.yaml` the main _Camel_ route. +
...
+
Append in your `store.camel.yaml` the following _Camel_ route. +
Copy the snippet below and paste it below your last route.
+
--
{copypaste}
----
...
1. Include the _Camel_ route that stores the data. 
+
...
1. Include a mock _Kafka_ producer
+
Now add a route that will permit you to test the behaviour of the system.
+
Append in your `store.camel.yaml` the following _Camel_ route. +
Copy the snippet below and paste it below your last route.
+
...
* A JSON body is configured to simulate an incoming chat message.
