...
. From your browser, use the invite URL below to join the lab room:
+
CAUTION: ensure you replace `YOUR_ROOM`
+
```http
https://matrix.to/#/#CamelWorkshop_YOUR_ROOM:gitter.im
```
+
{empty} +
+
...
+
----
https://matrix.to/#/#CamelWorkshop_room1:gitter.im
----
+
{empty} +
+
...
{blank}
+
The select `Continue in your browser`, as per the image below.
+
...
{blank}
+
To sign in, choose any of the single sign on options in the region highlighted in the image below:
+
image::images/matrix-join-4.png[align="left", width=50%]
+
{blank}
+
In your next step, choose you _Gitter_ (_Matrix_) username, and click `Continue`:
+
...
To find the room's ID you'll, follow the steps indicated below:
...
1. Initiate the authorisation flow
+
From your browser, enter the following URL:
+
```http
https://gitter.ems.host/_matrix/client/v3/login/sso/redirect?redirectUrl=http://localhost:8080
```
+
{blank}
+
...
1. Choose any of the given identity providers.
+
...
1. Take note of the returned `loginToken`

1. Request the authorization token
+
Exchange your `loginToken` to obtain an access token. +
Use the following cURL base command:
+
CAUTION: ensure you replace `YOUR_LOGIN_TOKEN` in the command below.
+
```curl
...
```
+
The above cURL command should return something like:
+
----
syt_Ym1lc2VndWUtZGVtby02MzE1ZjBlOTZkYTAzNzM5ODQ5YzQzM2Y_PvFLHTpEUVJNfHXYZkDO_0YDnHx
----
+
{empty} +
+
